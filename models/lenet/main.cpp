...
/// initialize.
GLOW_MEM_ALIGN(LENET_MEM_ALIGN)
uint8_t constantWeight[LENET_CONSTANT_MEM_SIZE] = {
#include "lenet.weights.txt"
};

/// Statically allocate memory for mutable weights (model input/output data).
GLOW_MEM_ALIGN(LENET_MEM_ALIGN)
uint8_t mutableWeight[LENET_MUTABLE_MEM_SIZE];

/// Statically allocate memory for activations (model intermediate results).
GLOW_MEM_ALIGN(LENET_MEM_ALIGN)
uint8_t activations[LENET_ACTIVATIONS_MEM_SIZE];

/// Bundle input data absolute address.
uint8_t *inputAddr = GLOW_GET_ADDR(mutableWeight, LENET_conv2d_input);

/// Bundle output data absolute address.
uint8_t *outputAddr = GLOW_GET_ADDR(mutableWeight, LENET_dense_2);
